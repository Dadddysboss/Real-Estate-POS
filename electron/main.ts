...
const REQUIRED_FIELD_DEFAULTS: Record<string, Record<string, string>> = {
  ...
  investors: { branch_id: 'BRANCH_MAIN' },
};
...
function sanitizeQueuedSQL(item: OfflineQueueItem): { sql: string; args: unknown[] } {
  ...
  if (insertMatch) {
    ...
    if (known) {
      ...
      columns.forEach((col, idx) => {
        if (known.includes(col)) {
          ...
          logToFile(`[Desktop Queue Sanitize] Dropping unknown column "${col}" from queued ${tableName} write`);
        }
      });
    ...
    if (defaults) {
      ...
      // No field defaults for this table — but if unknown columns were stripped above,
      // rebuild the SQL so it matches the filtered columns/args.
      const placeholderCount = (sql.match(/\?/g) || []).length;
      if (placeholderCount !== columns.length) {
        const placeholders = columns.map(() => '?').join(', ');
        sql = `INSERT OR IGNORE INTO ${tableName} (${columns.join(', ')}) VALUES (${placeholders})`;
      }
      return { sql, args };
...
async function dbExecute(sql: string, args: unknown[] = []): Promise<{ rows: Record<string, unknown>[] }> {
  ...
  try {
    ...
    if (isWrite) {
      ...
      // Surface queued state so the UI can show "saved offline" instead of false success
      return { rows: [], queued: true } as { rows: Record<string, unknown>[]; queued?: boolean };
    }
    throw err;
  }
}
...
const TABLES_TO_ENSURE = [
  ...
  "CREATE TABLE IF NOT EXISTS investors (id TEXT PRIMARY KEY, branch_id TEXT DEFAULT 'BRANCH_MAIN', pool_id TEXT NOT NULL, investor_name TEXT NOT NULL, phone_number TEXT, cnic TEXT, contributed_amount REAL DEFAULT 0, equity_percentage REAL DEFAULT 0, total_payout_received REAL DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))",
...
async function initializeDatabase() {
  try {
    ...
    const alterMigrations: [string, string, string][] = [
      ...
      // Office expenses / investors — branch_id must never be NULL (NOT NULL constraint crashes)
    ...
    try {
      ...
      if (hasTargetCapital && !hasTotalTargetCapital) {
        console.log('[Desktop] Detected outdated investor_pools schema (target_capital only) — recreating...');
        ...
        await dbExecuteMulti([
          ...
          { sql: "CREATE TABLE IF NOT EXISTS investors (id TEXT PRIMARY KEY, branch_id TEXT DEFAULT 'BRANCH_MAIN', pool_id TEXT NOT NULL, investor_name TEXT NOT NULL, phone_number TEXT, cnic TEXT, contributed_amount REAL DEFAULT 0, equity_percentage REAL DEFAULT 0, total_payout_received REAL DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))" },
      ...
      // Case 2: Both columns exist (corrupted/partial migration state) — recreate to clean up old column
        ...
        console.log('[Desktop] Detected investor_pools with both target_capital and total_target_capital columns — cleaning up...');
        await dbExecute("DROP TABLE IF EXISTS investor_payouts");
        await dbExecute("DROP TABLE IF EXISTS dividend_distributions");
        await dbExecute("DROP TABLE IF EXISTS investors");
        await dbExecute("DROP TABLE IF EXISTS investor_pools");
        ...
        console.log('[Desktop] Investor tables cleaned up (removed legacy target_capital column).');
      }
    } catch (e) {
